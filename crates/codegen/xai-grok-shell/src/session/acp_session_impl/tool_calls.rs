...
impl SessionActor {
    ...
    async fn apply_pre_tool_use_gate(
        ...
        let mut client_raw_input = raw_input.clone();
        ...
        if let Some(registry) = hook_registry_snapshot {
            ...
            if let Some(rewrite) = pre_result.updated_input {
                ...
                client_raw_input = validated.updated_json.clone();
        ...
        if let Some(denied) = self
            .run_pre_tool_use_client_hook(call, tool_call_id, &envelope, &client_raw_input)
            .await?
        {
