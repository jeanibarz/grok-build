...
pub async fn run_single_turn(
    ...
    // hooks) drains before the agent's `LocalSet` is dropped (via `AgentShutdownGuard`
    // / `cancel`), which would otherwise abort the actor. Without this the parent
    // session never emits `session_end` in headless `-p` mode (issue #1).
