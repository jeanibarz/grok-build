...
You can configure `[terminal] alt_screen` in `~/.grok/pager.toml`, pass
`--alt-screen` to force the alternate screen for a single session (useful for
remote supervisors under dtach / node-pty), or run `grok --no-alt-screen` to
confirm inline mode works. `--alt-screen` and `--no-alt-screen` are mutually
exclusive.
